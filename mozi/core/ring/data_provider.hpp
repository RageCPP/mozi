...
template <class I, typename Event> class mo_data_provider_c
{
    ...
    template <class U>
    struct event_get_sequence<U, std::enable_if_t<std::is_same_v<decltype(std::declval<U>()[size_t()]), const Event &>>>
        : std::true_type
    {
    };
  ...
  public:
    mo_data_provider_c()
    {
        static_assert(event_get_sequence<I>::value, "I should have `const Event& operator[](size_t)` method");
    }
};
