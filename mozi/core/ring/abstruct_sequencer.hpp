...
{
  public:
    mo_abstruct_sequencer_c(uint32_t buffer_size) : m_buffer_size(buffer_size)
    {
    }
    ~mo_abstruct_sequencer_c()
    {
        delete m_gating_sequences.load();
    }
    ...
    {
        return m_gating_sequences.load();
    }
  ...
  private:
    ...
    std::atomic<std::vector<mo_arc_sequence_t> *> m_gating_sequences{new std::vector<mo_arc_sequence_t>()};
};
} // namespace mozi::ring