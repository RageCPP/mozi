...
#define MO_EXPECTED_ID(X) MO_CONCATENATE(MO_CONCATENATE(MOZI, X), __LINE__)

#define MO_NEED_IMPL(...)                                                                                              \
    bool MO_EXPECTED_ID(Need) = false,                                                                                 \
         typename std::enable_if<(MO_EXPECTED_ID(Need) || static_cast<bool>(__VA_ARGS__)), int>::type = 0

#define MO_NEED_TRAILING(...) , MO_NEED_IMPL(__VA_ARGS__)

#define MO_NEED(...) template <MO_NEED_IMPL(__VA_ARGS__)>
...
template <class Error> class Unexpected final
{
  ...
  public:
    ...
    [[MO_COLD]] constexpr /* implicit */ Unexpected(const Error &err) : error_(err)
    {
    }
    [[MO_COLD]] constexpr /* implicit */ Unexpected(Error &&err) : error_(std::move(err))
    {
    }

    template <class Other MO_NEED_TRAILING(std::is_constructible<Error, Other &&>::value)>
     ...
     * Assignment
     */
    template <class Other MO_NEED_TRAILING(std::is_assignable<Error &, Other &&>::value)>
...
template <class Error MO_NEED_TRAILING(IsEqualityComparable<Error>::value)>
...
[[MO_NODISCARD]] constexpr Unexpected<typename std::decay<Error>::type> makeUnexpected(Error &&err)
{
...
template <> class MO_EXPORT BadExpectedAccess<void> : public std::exception
{
  public:
    ...
    BadExpectedAccess &operator=(BadExpectedAccess const &)
    {
        return *this;
    }
...
template <class Error> class MO_EXPORT BadExpectedAccess : public BadExpectedAccess<void>
{
  public:
    ...
    Error const &&error() const &&
    {
        return static_cast<Error const &&>(error_);
    }
...
template <template <class...> class Trait, class... Ts> using StrictAllOf = StrictConjunction<Trait<Ts>...>;

template <class T> using IsCopyable = StrictConjunction<std::is_copy_constructible<T>, std::is_copy_assignable<T>>;

template <class T> using IsMovable = StrictConjunction<std::is_move_constructible<T>, std::is_move_assignable<T>>;
...
template <class T, class U> auto doEmplaceAssign(int, T &t, U &&u) -> decltype(void(t = static_cast<U &&>(u)))
{
    t = static_cast<U &&>(u);
}
template <class T, class U> auto doEmplaceAssign(long, T &t, U &&u) -> decltype(void(T(static_cast<U &&>(u))))
{
...
template <class Value, class Error, StorageType = expected_detail::getStorageType<Value, Error>()> // ePODUnion
struct ExpectedStorage
{
    ...
    explicit constexpr ExpectedStorage(EmptyTag) noexcept : ch_{unsafe_default_initialized}, which_(Which::eEmpty)
    {
    }
    template <class... Vs>
    explicit constexpr ExpectedStorage(ValueTag, Vs &&...vs) noexcept(noexcept(Value(static_cast<Vs &&>(vs)...)))
        : value_(static_cast<Vs &&>(vs)...), which_(Which::eValue)
    {
    }
    template <class... Es>
    explicit constexpr ExpectedStorage(ErrorTag, Es &&...es) noexcept(noexcept(Error(static_cast<Es &&>(es)...)))
...
template <class Value, class Error> struct ExpectedUnion
{
    ...
    template <class... Vs>
    explicit constexpr ExpectedUnion(ValueTag, Vs &&...vs) noexcept(noexcept(Value(static_cast<Vs &&>(vs)...)))
        : value_(static_cast<Vs &&>(vs)...), which_(Which::eValue)
    {
    }
    template <class... Es>
    explicit constexpr ExpectedUnion(ErrorTag, Es &&...es) noexcept(noexcept(Error(static_cast<Es &&>(es)...)))
        : error_(static_cast<Es &&>(es)...), which_(Which::eError)
    {
    }
...
template <class Derived, bool Noexcept> struct MoveConstructible<Derived, false, Noexcept>
{
    constexpr MoveConstructible() = default;
    constexpr MoveConstructible(const MoveConstructible &) = default;
...
template <class Derived, bool Noexcept> struct CopyAssignable<Derived, false, Noexcept>
{
    constexpr CopyAssignable() = default;
    constexpr CopyAssignable(const CopyAssignable &) = default;
    constexpr CopyAssignable(CopyAssignable &&) = default;
...
template <class Derived, bool Noexcept> struct MoveAssignable<Derived, false, Noexcept>
{
    constexpr MoveAssignable() = default;
    constexpr MoveAssignable(const MoveAssignable &) = default;
    constexpr MoveAssignable(MoveAssignable &&) = default;
...
struct ExpectedStorage<Value, Error, StorageType::eUnion>
    : ExpectedUnion<Value, Error>,
      ...
      CopyAssignable<ExpectedStorage<Value, Error, StorageType::eUnion>, StrictAllOf<IsCopyable, Value, Error>::value,
                     StrictAllOf<IsNothrowCopyable, Value, Error>::value>,
      MoveAssignable<ExpectedStorage<Value, Error, StorageType::eUnion>, StrictAllOf<IsMovable, Value, Error>::value,
...
template <class Value, class Error> struct ExpectedStorage<Value, Error, StorageType::ePODStruct>
{
    using value_type = Value;
    using error_type = Error;
    Which which_;
    ...
    explicit constexpr ExpectedStorage(EmptyTag) noexcept : which_(Which::eEmpty), error_{}, value_{}
    {
    }
    template <class... Vs>
    explicit constexpr ExpectedStorage(ValueTag, Vs &&...vs) noexcept(noexcept(Value(static_cast<Vs &&>(vs)...)))
...
struct ExpectedHelper
{
    ...
    static auto then_(This &&ex, Fn &&fn, Fns &&...fns)
        -> decltype(T::then_(T::template return_<E>((std::declval<Fn>()(std::declval<This>().value()), unit)),
                             std::declval<Fns>()...))
    {
        if (MO_LIKELY(ex.which_ == expected_detail::Which::eValue))
        {
    ...
    template <class This, class Yes, class No, class Ret = decltype(std::declval<Yes>()(std::declval<This>().value())),
    ...
    {
        using Which = expected_detail::Which;
        if (ex.which_ == Which::eValue) [[MO_LIKELY]]
        {
    ...
    static Ret thenOrThrow_(This &&ex, Yes &&yes, No &&no)
    {
        ...
        throw_exception<BadExpectedAccess<ExpectedErrorType<This>>>(static_cast<This &&>(ex).error());
    }
     ...
     */
    template <
        class This, class No, class... AndFns, class E = ExpectedErrorType<This>, class V = ExpectedValueType<This>,
        class T = ExpectedHelper,
        class RetValue = decltype(T::then_(T::template return_<E>((std::declval<No>()(std::declval<This>().error()))),
                                           std::declval<AndFns>()...)
                                      .value()),
        typename std::enable_if<!std::is_same<std::remove_reference<RetValue>,
    ...
    static auto orElse_(This &&ex, No &&no, AndFns &&...fns) -> Expected<V, E>
    {
        ...
        return T::then_(T::template return_<E>((static_cast<No &&>(no)(static_cast<This &&>(ex).error()))),
     ...
     */
    template <
        class This, class No, class... AndFns, class E = ExpectedErrorType<This>, class T = ExpectedHelper,
        class RetValue = decltype(T::then_(T::template return_<E>((std::declval<No>()(std::declval<This>().error()))),
                                           std::declval<AndFns>()...)
                                      .value()),
        typename std::enable_if<std::is_same<std::remove_reference<RetValue>,
     ...
     * func.
     */
    template <class This, class No, class... AndFns, class E = ExpectedErrorType<This>,
...
template <class Value, class Error> class Expected final : expected_detail::ExpectedStorage<Value, Error>
{
  ...
  public:
    ...
    template <class... Ts MO_NEED_TRAILING(std::is_constructible<Value, Ts &&...>::value)> void emplace(Ts &&...ts)
    {
    ...
    {
        if (expected_detail::Which::eValue == this->which_) [[MO_LIKELY]]
        {
            return true;
        }
        return false;
    ...
    {
        if (expected_detail::Which::eError == this->which_) [[MO_UNLIKELY]]
        {
            return true;
        }
        return false;
    ...
    {
        if (this->which_ == expected_detail::Which::eValue) [[MO_LIKELY]]
        {
            return this->value_;
        }
     ...
     * then
     */
    template <class... Fns MO_NEED_TRAILING(sizeof...(Fns) >= 1)>
    auto then(Fns &&...fns) const & -> decltype(expected_detail::ExpectedHelper::then_(std::declval<const Base &>(),
                                                                                       std::declval<Fns>()...))
    {
    ...
    template <class... Fns MO_NEED_TRAILING(sizeof...(Fns) >= 1)>
    auto then(Fns &&...fns) & -> decltype(expected_detail::ExpectedHelper::then_(std::declval<Base &>(),
                                                                                 std::declval<Fns>()...))
    {
        if (this->uninitializedByException())
        {
                                                                                  ...
                                                                                  std::declval<Fns>()...))
    {
        ...
        return expected_detail::ExpectedHelper::then_(std::move(base()), static_cast<Fns &&>(fns)...);
    }
     ...
     */
    template <class... Fns MO_NEED_TRAILING(sizeof...(Fns) >= 1)>
    auto orElse(Fns &&...fns) const & -> decltype(expected_detail::ExpectedHelper::orElse_(std::declval<const Base &>(),
                                                                                           std::declval<Fns>()...))
    {
        if (this->uninitializedByException())
        {
    ...
    template <class... Fns MO_NEED_TRAILING(sizeof...(Fns) >= 1)>
    auto orElse(Fns &&...fns) & -> decltype(expected_detail::ExpectedHelper::orElse_(std::declval<Base &>(),
                                                                                     std::declval<Fns>()...))
    {
        if (this->uninitializedByException())
        {
                                                                                      ...
                                                                                      std::declval<Fns>()...))
    {
        ...
        return expected_detail::ExpectedHelper::orElse_(std::move(base()), static_cast<Fns &&>(fns)...);
    }
    ...
    auto thenOrThrow(Yes &&yes, No &&no = No{}) const & -> decltype(std::declval<Yes>()(std::declval<const Value &>()))
    {
        ...
        return Ret(
            expected_detail::ExpectedHelper::thenOrThrow_(base(), static_cast<Yes &&>(yes), static_cast<No &&>(no)));
    }
    ...
    auto thenOrThrow(Yes &&yes, No &&no = No{}) && -> decltype(std::declval<Yes>()(std::declval<Value &&>()))
    {
        ...
        return Ret(expected_detail::ExpectedHelper::thenOrThrow_(std::move(base()), static_cast<Yes &&>(yes),
                                                                 static_cast<No &&>(no)));
    }
  ...
  private:
    ...
    {
        if (!hasValue()) [[MO_UNLIKELY]]
        {
            if (hasError()) [[MO_LIKELY]]
            {
    ...
    template <typename Self> static void requireValueMove(Self &self)
    {
        if (!self.hasValue()) [[MO_UNLIKELY]]
        {
            if (self.hasError()) [[MO_LIKELY]]
            {
    ...
    {
        return requireValueMove(*this);
    }
    void requireValueMove() const
    {
        return requireValueMove(*this);
    }
    ...
    const Expected<Value, Error> &lhs, const Expected<Value, Error> &rhs)
{
    ...
    if (lhs.which_ != rhs.which_) [[MO_UNLIKELY]]
    {
        return false;
    }
    if (lhs.hasError()) [[MO_UNLIKELY]]
